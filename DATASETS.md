<h1 id="dataset"><a href="https://github.com/asif-hanif/zebra">ZEBRA</a> Instructions for Dataset Preparation</h1>

This document provides instructions on how to prepare the datasets for training and testing the models. The datasets used in [ZEBRA](https://github.com/asif-hanif/zebra) project are as follows: 
...
**Base and Novel Classes**  For the generalized few-shot setup used in this project, each dataset is first split into two disjoint subsets: `base` and `novel` classes. The split is created by shuffling all class names for a dataset, taking the first half as base classes and the second half as novel classes. This is implemented in [scripts/base-novel-classes/get_base_novel_classes.py](scripts/base-novel-classes/get_base_novel_classes.py), which saves one JSON file per dataset in [scripts/base-novel-classes](scripts/base-novel-classes), such as [scripts/base-novel-classes/ESC50_BaseNovelClassnames.json](scripts/base-novel-classes/ESC50_BaseNovelClassnames.json). Each JSON file contains three lists:

- `base`: classes used for supervised adaptation / training,
- `novel`: classes used for evaluation on unseen classes,
