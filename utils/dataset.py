...
class FewShotDataset(Dataset):
    def __init__(self, root, split=None, num_shots=-1, repeat=False, process_audio_fn=None, resample=True, classnames=None): 
        """
        Args:
        ...
        # assert if given classnames are in the dataframe
        assert classnames is None or all([classname in df['classname'].unique().tolist() for classname in classnames]), "Some given classnames are not present in the dataset.\nClassnames in dataset: {}\nGiven classnames: {}".format(df['classname'].unique().tolist(), classnames)
        ...
        # get only the given classnames samples from df
        df = df[df['classname'].isin(classnames)].reset_index(drop=True)
        ...
        if self.split == 'train': print("\n\nIMPORTANT: Training will be done only on base classes.\n")
        print("########################################################\n\n")
